...
def PlotRegion(region) :
    ...
    for filename in filenames :
        if filename in par and region in par[filename] :
            data = pd.read_csv(Path(folder+'/'+filename+'_'+str(par[filename][region]['Channel'])+'.csv'))
            for idx, col in enumerate(data) :
                if idx == 0 :
                    x = data[col]
                    if 'xOffset' in par[filename] :
                        x = BE(x,par[filename]['xOffset'])
                elif idx == 1 :
                    y = data[col]
                else :
                    y += data[col]
...
def PlotFile(filename,region,runs=range(0,10000),normalize=False) :
